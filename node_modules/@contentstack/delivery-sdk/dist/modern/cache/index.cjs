...
async function handleRequest(cacheOptions, apiKey, defaultAdapter, resolve, reject, config) {
  if (!cacheOptions.persistenceStore) {
    throw new Error(errorMessages.ErrorMessages.MISSING_PERSISTENCE_STORE);
  }
  ...
  switch (cacheOptions.policy) {
    case types.Policy.NETWORK_ELSE_CACHE: {
    ...
    case types.Policy.CACHE_THEN_NETWORK: {
      const cacheResponse = cacheStore.getItem(enhancedCacheKey, config.contentTypeUid);
      if (cacheResponse)
        return resolve({
          data: cacheResponse,
    ...
    case types.Policy.CACHE_ELSE_NETWORK: {
      const cacheResponse = cacheStore.getItem(enhancedCacheKey, config.contentTypeUid);
      if (cacheResponse)
        return resolve({
          data: cacheResponse,
